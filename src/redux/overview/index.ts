...
import { ReviewMaster, ReviewSchema, Review, User } from "polyvolve-ui/lib/@types"
import { sortByOrder, sortBySurname } from "polyvolve-ui/lib/utils/sort"
import { take, call, put } from "redux-saga/effects"
import { API_URL } from "../../constants/env";
import { axios, defaultHeaders, getErrorMessage, authenticatedHeader } from "../../lib/axios";
...
const actions = {
  ...
  getReviewDataResponse: createAction<ReviewMaster>(actionNames.GET_REVIEW_DATA_RESPONSE),
  getOrCreateReviewRequest: createAction<{}>(actionNames.CREATE_REVIEW_REQUEST),
  getOrCreateReviewResponse: createAction<Review>(actionNames.CREATE_REVIEW_RESPONSE),
}
...
export function* handleGetReviewData() {
  while (true) {
    ...
    const master: ReviewMaster = res.data.data.master
    master.reviewedUsers = res.data.data.reviewedUsers.sort(sortBySurname)
    ...
    yield put(actions.getReviewDataResponse({ error: "", master, user, schema }))
  }
}
...
export function* handleGetOrCreateReview() {
  while (true) {
    ...
    if (err) {
      yield put(actions.getOrCreateReviewResponse({ error: getErrorMessage(err) }))
      continue
    }
    ...
    const review: Review = res.data.data
